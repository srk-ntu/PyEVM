...
def rgb2ntsc(src):
  ...
  T = np.array([[0.114, 0.587, 0.298], [-0.321, -0.275, 0.596],
                [0.311, -0.528, 0.212]])
  for i in range(rows):
    for j in range(cols):
...
def load_video(video_filename, levels, window_size = 400):
  HEIGHT = 480
  WIDTH = 640
  ...
  video_tensor = np.zeros((window_size, resized_ht, resized_wt, 3),
                          dtype='float')
  x = 0
  while cap.isOpened() and x<50:
    ret, frame = cap.read()
    if ret is True:
...
def gaussian_video(video_tensor, levels=3):
  for i in range(0, video_tensor.shape[0]):
    ...
    if i == 0:
      vid_data = np.zeros((video_tensor.shape[0], gaussian_frame.shape[0],
...
def save_video(video_tensor, saved_name, modew='motion'):
  ...
  writer = cv2.VideoWriter(saved_name + '_' + modew + '.avi', fourcc, 30,
                           (width, height), 1)
  for i in range(0, video_tensor.shape[0]):
...
def magnify_color(video_name, hps, saved_name=None):
  ...
  filtered_tensor = temporal_ideal_filter(gau_video, hps.cutoff_low,
                                          hps.cutoff_high, f)
  amplified_video = amplify_video(filtered_tensor,
                                  amplification=hps.amplification)
...
def magnify_motion(video_name, hps, saved_name=None):
  ...
  for i in range(hps.levels):
    filter_tensor = butter_bandpass_filter(lap_video_list[i], hps.cutoff_low,
                                           hps.cutoff_high, f)
